"""
hynix_long_collector.py — KODEX SK하이닉스단일종목레버리지(0193T0) 가격 수집.

Enhanced 자동매매의 상승(UP/PRIMARY_TREND_UP) 신호는 SK하이닉스(000660)를 직접
매수하지 않고, 이 레버리지 ETF를 실제 거래 종목으로 매수한다(000660은 시세·추세·
신호 계산에만 쓰인다). app.data_sources.hynix_inverse_collector(0197X0)와 동일한
우선순위(KIS → Naver → 캐시)를 따른다. 실패해도 예외를 던지지 않고 최근 캐시값 +
...
# 레버리지 단일종목 ETF의 가격대는 확정하기 어려우므로(신규/변동 상장가) 넓게 잡되,
# 0원/음수/명백한 이상치(코스피 전체 지수 수준 등)만 걸러낸다.
LONG_PRICE_MIN = 100
...
def _valid_long_price(price: object) -> bool:
    try:
        value = float(price)
    except (TypeError, ValueError):
        return False
    return LONG_PRICE_MIN <= value <= LONG_PRICE_MAX
...
def _fetch_naver_long_current(code: str = LONG_SYMBOL) -> dict:
    """Naver Finance에서 레버리지 ETF 현재가 조회(하이닉스 전용 가격범위 검증 회피)."""
    ...
    try:
        ...
        if not _valid_long_price(price):
        ...
        logger.debug("[HynixLong] Naver 현재가 수집 실패: %s", exc)
...
def _write_json_cache(payload: dict) -> None:
    try:
        ...
        logger.debug("[HynixLong] 현재가 캐시 저장 실패: %s", exc)
...
def collect_long_current(mode: Optional[str] = None) -> dict:
    """레버리지 ETF(0193T0) 현재가 수집. 우선순위: KIS → Naver → 최근 캐시(stale=True).

    종목코드는 그대로(문자 포함) 사용한다 — isdigit()==False라서 벌어지는 ETF 필터
    오탐(2026-07-14 이전 인버스 종목에서 이미 확인된 문제)을 그대로 반복하지 않도록,
    숫자형 변환이나 코드 문자 제거를 시도하지 않는다.
    """
    mode = mode or _kis_mode()
    result = {
        "symbol": LONG_SYMBOL, "name": LONG_NAME,
    ...
    if mode:
        try:
            ...
            result["error"] = f"KIS 레버리지 현재가 실패: {exc}"
    else:
        result["error"] = "KIS 인증 정보 없음 (mock/real 모두 미설정)"

    naver = _fetch_naver_long_current()
...
def _save_long_minute(df: pd.DataFrame) -> None:
    try:
        CACHE_DIR.mkdir(parents=True, exist_ok=True)
        df.to_csv(_MINUTE_CSV, index=False, encoding="utf-8-sig")
    except Exception as exc:
        logger.debug("[HynixLong] 분봉 캐시 저장 실패: %s", exc)


def _load_long_minute_cache() -> Optional[pd.DataFrame]:
    try:
...
def collect_long_minute(mode: Optional[str] = None, count: int = 60) -> dict:
    """레버리지 ETF(0193T0) 1분봉 수집. KIS만 지원."""
    mode = mode or _kis_mode()
    result = {
        ...
        try:
            ...
            if candles:
                ...
                if not df_1min.empty:
                    _save_long_minute(df_1min)
                    result.update(
                        df_1min=df_1min, source="kis", status="success",
                        last_bar_time=df_1min["datetime"].iloc[-1].isoformat(),
                    )
                    return result
            result["error"] = "KIS 레버리지 분봉 응답 없음"
        except Exception as exc:
            result["error"] = f"KIS 레버리지 분봉 실패: {exc}"
    else:
        result["error"] = "KIS 인증 정보 없음 (mock/real 모두 미설정)"

    cached = _load_long_minute_cache()
