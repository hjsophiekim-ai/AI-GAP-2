...
MACD2's own state file (the same pattern MACD2's service.py already used for
its one-way check).
...
def _age_sec(iso_ts: Optional[str]) -> Optional[float]:
    ...
    Handles both naive (Enhanced, ``datetime.now().isoformat()``) and
    tz-aware (MACD2, ``datetime.now(KST).isoformat()``) timestamps.
    """
    if not iso_ts:
        return None
    try:
