...
class RuntimeState:
    ...
    # X2-lite (2026-09-12) — TW TEG 3-SLOT 과 진입이 100% 동일한 자매 전략.
    # 위 tw2_3slot_* 슬롯/후보/진단 필드를 그대로 공유하고(세 토글은 상호배타),
    # 갈라지는 것은 time_window_3slot.exit_overrides /
    # morning_tp2_pct_override 가 주는 청산 임계값과 내장 조기익절뿐이다.
    # 새 슬롯 카운터/후보 필드를 하나도 추가하지 않는다.
    time_window_x2lite_filter_enabled: bool = False
    time_window_x2lite_filter_enabled_at: Optional[str] = None
