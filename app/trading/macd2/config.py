...
# 에는 관여하지 않는다(청산 전용).
#
# ── 2026-09-17 재검증 결과: 기본 OFF (채택기준 미달) ─────────────────────────
# 76영업일 faithful-fill 재검증(20260604~20260916, X2-lite 청산 + W1a 사이징,
# 진입게이트/플래그 불변). A = X2-lite+W1a / B = 기존 H50 / C = H50+watch:
#
#            70일 복리    PF      MDD     Top10제외  |  30일 복리   PF      MDD
#   A       +176.32%   2.1548  -9.68     +62.59    |   +58.51%  2.0896  -5.94
#   B       +204.18%   2.2165  -7.32     +78.98    |   +64.60%  2.1722  -7.20
#   C       +193.51%   2.2633  -8.06     +72.71    |   +61.13%  2.1665  -5.94
#
# C 는 A 는 이기지만 **B 보다 30일·70일 복리가 둘 다 낮고 Top10 제외도 열위**라
# 사용자 채택기준("기존 H50 보다 30일/70일 둘 다 비악화 또는 개선")을 넘지
# 못한다. 원인은 구조적이다 — whipsaw-watch 는 "gap 이 확대되지 않아서" 보류한
# TW 분기의 후속 확인기로 설계됐는데, H50 은 gap 과 무관하게(추세 정렬 + 좁은
# rng60) 보류하므로 이미 강하게 확대 중인 gap 위에서 같은 규칙을 걸면 거의
# 곧바로 발화한다: 70일 watch 청산이 B 1건 -> C 23건. 20건의 청산사유 변경 중
# 개선 10 / 악화 10 이고 합이 음수다.
#
# 얻는 것도 있다 — PF 2.2165 -> 2.2633, 30일 MDD -7.20 -> -5.94, 하드스톱
# 70일 48 -> 40건. 그리고 2026-09-16 사건 자체는 개선된다(13:54 BREAKEVEN_STOP
# +0.199% -> 13:36 watch 청산 +0.537%, 18분 단축). 즉 "수익을 늘리는 변경"이
# 아니라 "위험을 줄이는 변경"이다. 켤지 말지는 그 교환을 받아들이느냐의 문제라
# 사용자 결정 사항으로 남긴다.
#
# 코드/테스트/검증기록은 지우지 않고 토글만 내린다(2026-09-07 Slot1 CHOP veto
# 와 같은 처리). 켜려면 MACD2_H50_WHIPSAW_WATCH_ENABLED=1, 영구 채택하려면 이
# 기본값을 True 로 바꾸고 이 표를 갱신할 것.
H50_WHIPSAW_WATCH_ENABLED = _env_bool("MACD2_H50_WHIPSAW_WATCH_ENABLED", False)
