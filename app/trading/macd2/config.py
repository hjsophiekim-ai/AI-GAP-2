...
# 2026-09-16 실거래에서 H50 이 12:48 RED 진입분을 13:33 에 HOLD 한 뒤 BLUE 방향
# MACD gap 이 -58 -> -495 -> -855 -> -1018 -> -1106 으로 확대되는 동안 계속
# 보유했다. 해제조건이 EMA20/50 2봉 이탈과 60분 경과뿐이라 "보류가 틀렸는지"
# 확인할 장치가 없기 때문이다. 그 장치로 TW2/TW2 3-SLOT 의 whipsaw-watch 를
# H50 HOLD 에도 붙이는 안(`MACD2_H50_WHIPSAW_WATCH_ENABLED`)을 구현·검증했으나
# **채택하지 않고 코드에서 제거**했다. 76영업일 faithful-fill 재검증:
#
#            70일 복리    PF      MDD    Top10제외 |  30일 복리   PF      MDD
#   A(X2-lite) +176.32%  2.1548  -9.68    +62.59  |   +58.51%  2.0896  -5.94
#   B(H50)     +204.18%  2.2165  -7.32    +78.98  |   +64.60%  2.1722  -7.20
#   C(H50+watch) +193.51% 2.2633 -8.06    +72.71  |   +61.13%  2.1665  -5.94
#
# 원인은 구조적이다 -- whipsaw-watch 는 "gap 이 확대되지 않아서" 보류한 TW 분기의
# 후속 확인기라 평평한 gap 출발을 전제하는데, H50 은 gap 과 무관하게(추세 정렬 +
# 좁은 rng60) 보류하므로 이미 확대 중인 gap 위에 같은 규칙을 걸면 대부분 다음 한
# 봉에서 바로 발화한다(70일 watch 청산 1건 -> 23건, H50 고유 해제 8건 -> 2건).
# 연결이 H50 을 보조하지 않고 대체해 버린다.
#
# 따라서 **H50 의 해제경로는 `worker._advance_h50_hold` 의 두 가지뿐이다** --
# 구조추세 2봉 이탈 / 최대 60분. H50 HOLD 분기에서 `_start_whipsaw_watch` 를
# 부르지 말 것. TW2·TW2 3-SLOT 자신의 whipsaw-watch 는 이 결정과 무관하게 예전
# 그대로 동작한다. 연구기록 전량:
# `data/validation/macd2/h50_whipsaw_watch_20260917/README.md`.
...
# 2026-09-21: H50 해제 청산은 실제 MACD 반대신호가 아니다. 원장에서
# 구분되도록 전용 사유를 둔다(기존 SMALL_WHIPSAW_HOLD_EXIT 은 신호원장
# block_reason 으로 그대로 남는다).
EXIT_H50_TREND_BREAK = "H50_WHIPSAW_RELEASE"
EXIT_H50_MAX_HOLD = "H50_MAX_HOLD_EXIT"
...
# ── X1 CONTEXT (2026-09-23, feature/macd2-x1-context-filter) ───────────────
# 새 MACD 신호를 만들지 않는다. 기존 RED/BLUE 신호를 입력으로 받아 "지금 맥락에서
# 이 신호를 어떻게 다룰지"만 답하는 보조필터다. 주문금액도 계산하지 않는다
# (SMART sizing 은 X1 이 ENTRY/REENTRY/LATE_ENTRY 를 허용한 뒤에만 돈다).
#
# 기본 OFF. X1_ENABLED 는 N1 과 C1 이 **모두** 켜져 있을 때만 유효하다
# (x1_context.x1_active() 가 그 조건을 강제한다). SMART 와는 독립이다.
#
# 단계 1 은 **판단 모듈 + shadow 기록만** 이다 — 주문경로에 X1 action 을
# 적용하지 않는다. X1_SHADOW_MODE 만 켜면 실제 주문은 그대로 두고
# x1_shadow_ledger.csv 에 would_* 판정만 남는다.
X1_ENABLED = _env_bool("MACD2_X1_ENABLED", False)
X1_SHADOW_MODE = _env_bool("MACD2_X1_SHADOW_MODE", False)
X1_FILTER_VERSION = "X1_CONTEXT_V1_20260923"
X1_STRATEGY_NAME = "X1_CONTEXT"
X1_SHADOW_LEDGER_FILENAME = "x1_shadow_ledger.csv"

# ── X1-1 MORNING CONTEXT ───────────────────────────────────────────────────
# 점수는 **연구용 초기값**이다. 새 threshold 최적화 금지(2026-09-23 사용자 지시).
X1_MORNING_W_PREMARKET_TREND_ALIGN = _env_int("MACD2_X1_W_PRE_ALIGN", 2)
X1_MORNING_W_SESSION_TREND_ALIGN = _env_int("MACD2_X1_W_SESS_ALIGN", 1)
X1_MORNING_W_GAP_EXPANDING = _env_int("MACD2_X1_W_GAP", 1)
X1_MORNING_W_EMA20_SLOPE_ALIGN = _env_int("MACD2_X1_W_EMA20", 1)
X1_MORNING_W_VWAP_FAVORABLE = _env_int("MACD2_X1_W_VWAP", 1)
X1_MORNING_W_PREMARKET_TREND_AGAINST = _env_int("MACD2_X1_W_PRE_AGAINST", -2)
X1_MORNING_W_SHORT_REVERSAL_ONLY = _env_int("MACD2_X1_W_SHORT_REV", -1)
X1_MORNING_W_MIDRANGE_PULLBACK = _env_int("MACD2_X1_W_MIDRANGE", -1)
#: score >= PASS -> 기존 진입 그대로 / 0..PASS-1 -> PASS_WEAK(로그만) / <0 -> WATCH
X1_MORNING_PASS_SCORE = _env_int("MACD2_X1_MORNING_PASS_SCORE", 2)
#: WATCH 유효시간(분). 이 안에 원래 방향으로 swing/box breakout 이 나오면 재승인.
X1_MORNING_WATCH_MAX_MIN = _env_int("MACD2_X1_MORNING_WATCH_MAX_MIN", 9)
#: "짧은 반전"과 "큰 추세" 를 가르는 룩백(분). 기존 관례값 재사용, 새 최적화 아님.
X1_MORNING_SHORT_LOOKBACK_MIN = _env_int("MACD2_X1_MORNING_SHORT_MIN", 15)
X1_MORNING_LONG_LOOKBACK_MIN = _env_int("MACD2_X1_MORNING_LONG_MIN", 60)
#: day range 중앙 되돌림 판정 밴드(퍼센타일). 0.35~0.65 = 중앙 30%.
X1_MORNING_MIDRANGE_LO_PCTL = _env_float("MACD2_X1_MIDRANGE_LO", 0.35)
X1_MORNING_MIDRANGE_HI_PCTL = _env_float("MACD2_X1_MIDRANGE_HI", 0.65)

# ── X1-2 FLIP EXIT ─────────────────────────────────────────────────────────
#: "플래그 3개"가 아니라 **방향전환 3회**.
X1_FLIP_EXIT_MIN_FLIPS = _env_int("MACD2_X1_FLIP_EXIT_MIN_FLIPS", 3)
X1_EXIT_W_BOX_BREAK = _env_int("MACD2_X1_EXIT_W_BOX", 2)
X1_EXIT_W_GAP_EXPANDING = _env_int("MACD2_X1_EXIT_W_GAP", 1)
X1_EXIT_W_EMA_SLOPE_FLIP = _env_int("MACD2_X1_EXIT_W_EMA", 1)
X1_EXIT_W_ETF_FOLLOW = _env_int("MACD2_X1_EXIT_W_ETF", 1)
X1_EXIT_W_EXTREME_FAIL = _env_int("MACD2_X1_EXIT_W_EXTREME", 1)
X1_FLIP_EXIT_SCORE_MIN = _env_int("MACD2_X1_FLIP_EXIT_SCORE_MIN", 3)

# ── X1-3 AFTERNOON RE-ENTRY (AR1) ──────────────────────────────────────────
# AR1 은 **narrow exception only** — 새 numerical threshold 를 만들지 않는다.
# 적용대상: resolve_slot 이 SAME_DIRECTION_AFTERNOON 으로 거절한 후보뿐.
# TEG 탈락이 price_ema_stack_aligned 하나뿐이고 macd_gap/ema_spread 확대 +
# vwap 우호가 전부 참일 때만 stack 을 면제한다. PLB(오후 TEG 전체 면제) 금지.
X1_AR1_ENABLED = _env_bool("MACD2_X1_AR1_ENABLED", True)

# ── X1-4 FLIP BREAKOUT WATCH / LATE ENTRY ──────────────────────────────────
#: 짧은 창 하나로 제한하지 않는다 — 두 조건 중 하나만 맞아도 WATCH.
X1_FLIPWATCH_WINDOW_A_MIN = _env_int("MACD2_X1_FLIPWATCH_WIN_A", 60)
X1_FLIPWATCH_MIN_FLIPS_A = _env_int("MACD2_X1_FLIPWATCH_FLIPS_A", 3)
X1_FLIPWATCH_WINDOW_B_MIN = _env_int("MACD2_X1_FLIPWATCH_WIN_B", 90)
X1_FLIPWATCH_MIN_FLAGS_B = _env_int("MACD2_X1_FLIPWATCH_FLAGS_B", 5)
X1_LATE_W_BOX_BREAKOUT = _env_int("MACD2_X1_LATE_W_BOX", 2)
X1_LATE_W_GAP_EXPANDING = _env_int("MACD2_X1_LATE_W_GAP", 1)
X1_LATE_W_NEXT_BAR_HOLD = _env_int("MACD2_X1_LATE_W_HOLD", 1)
X1_LATE_W_ETF_CONFIRM = _env_int("MACD2_X1_LATE_W_ETF", 1)
X1_LATE_W_EMA_SPREAD = _env_int("MACD2_X1_LATE_W_EMASPREAD", 1)
X1_LATE_W_VWAP = _env_int("MACD2_X1_LATE_W_VWAP", 1)
X1_LATE_ENTRY_SCORE_MIN = _env_int("MACD2_X1_LATE_SCORE_MIN", 4)
#: provisional guard — timeout 은 hard-code 하지 말라는 지시에 따라 research trace
#: 에는 3/6/9/12/15/30 버킷을 전부 남기고, **production 행동만** 이 값으로 막는다.
X1_LATE_ENTRY_MAX_LATENCY_MIN = _env_int("MACD2_X1_LATE_MAX_LATENCY_MIN", 15)
X1_LATENCY_BUCKETS_MIN = (3, 6, 9, 12, 15, 30)

# ── X1-5 AFTERNOON BLUE BONUS ──────────────────────────────────────────────
# hard rule 이 아니라 **가점 +1** 뿐이다. 13시 이후 BLUE 라고 무조건 주지 않는다.
X1_AFTERNOON_BLUE_BONUS = _env_int("MACD2_X1_AFT_BLUE_BONUS", 1)
