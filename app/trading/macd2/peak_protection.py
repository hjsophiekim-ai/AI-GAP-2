...
def is_supported_mode(state) -> bool:
    ...
    C1 은 TP2 가 8% 까지 열리는 **N1 래더 전용** overlay 다 — peak 5% 도달
    후 TP2 8% 사이의 무보호 구간을 메우는 규칙이기 때문이다. TP2 가 5.0% 인
    X2-lite/H50 에서는 peak 5% 도달 즉시 TP2 가 전량청산하므로 C1 은 구조적으로
    발동할 일이 없다(2026-09-19 연구의 이식검증에서 발동 0건 확인).

    2026-09-20: N1 이 production 모드로 들어오면서 허용 모드를 **N1 만으로
    좁혔다**. 그 전에는 X2-lite 계열에서도 켤 수 있었지만(발동은 안 됐다),
    "켤 수는 있는데 아무 일도 안 한다" 는 상태를 UI 에 남겨 둘 이유가 없다.
    """
