...
@dataclass(frozen=True)
class ConfirmedColorFlagResult:
    """Stateful KIS color-flag publication decision for a completed 3m bar."""

    raw_color: Direction
    previous_color_state: Optional[Direction]
    current_color_state: Optional[Direction]
    direction: Direction
    onset: bool
...
def _color_publication_regime(macd_snapshot: MacdSnapshot, raw_color: Direction) -> tuple[Optional[str], int]:
    """Return the KIS color-state regime and required stable raw bars.
...
def evaluate_confirmed_macd_color_onset(
    macd_snapshot: MacdSnapshot,
    previous_color_state: Optional[Direction],
    ...
    *,
...
) -> ConfirmedColorFlagResult:
    """Stateful KIS color-state onset for completed 3m MACD bars.

    This separates raw bar color (`confirmed_macd_flag_condition`) from the
    single order-authoritative flag emitted when KIS's color state changes.
    """
    ...
    if raw_color == Direction.HOLD or regime is None:
        return ConfirmedColorFlagResult(
            raw_color=raw_color,
            previous_color_state=previous_color_state,
            current_color_state=previous_color_state,
    ...
    onset = (
        publishable
        and previous_color_state != raw_color
        and next_count >= required_count
    )
    ...
    return ConfirmedColorFlagResult(
        ...
        direction=direction,
        onset=onset,
        pending_direction=None if onset else raw_color,
        pending_count=0 if onset else next_count,
        required_count=required_count,
        regime=regime,
        publishable=publishable,
    )
