...
def serialize(state: RuntimeState) -> dict[str, Any]:
    ...
    return {
        ...
        "last_resolve_error": state.last_resolve_error,
        ...
        "evaluated_bar_ts_today": list(state.evaluated_bar_ts_today or []),
        ...
        "daily_total_entry_count": int(state.daily_total_entry_count or 0),
        ...
        "x1_flag_history": list(getattr(state, "x1_flag_history", None) or []),
        "x1_flip_exit_armed": bool(getattr(state, "x1_flip_exit_armed", False)),
        "x1_flip_exit_armed_at": getattr(state, "x1_flip_exit_armed_at", None),
        "x1_flip_exit_owner_epoch": int(getattr(state, "x1_flip_exit_owner_epoch", 0) or 0),
        "x1_last_checked_bar_ts": getattr(state, "x1_last_checked_bar_ts", None),
        "x1_last_action": getattr(state, "x1_last_action", None),
        "x1_last_action_at": getattr(state, "x1_last_action_at", None),
        "last_x1_trace": getattr(state, "last_x1_trace", None),
...
def deserialize(raw: dict[str, Any]) -> RuntimeState:
    ...
    return RuntimeState(
        ...
        last_resolve_error=raw.get("last_resolve_error"),
        last_resolve_error_at=raw.get("last_resolve_error_at"),
        ...
        evaluated_bar_ts_today=[str(x) for x in (raw.get("evaluated_bar_ts_today") or []) if x],
        ...
        daily_total_entry_count=int(raw.get("daily_total_entry_count") or 0),
        ...
        x1_flag_history=list(raw.get("x1_flag_history") or []),
        x1_flip_exit_armed=bool(raw.get("x1_flip_exit_armed", False)),
        x1_flip_exit_armed_at=raw.get("x1_flip_exit_armed_at"),
        x1_flip_exit_owner_epoch=int(raw.get("x1_flip_exit_owner_epoch", 0) or 0),
        x1_last_checked_bar_ts=raw.get("x1_last_checked_bar_ts"),
        x1_last_action=raw.get("x1_last_action"),
        x1_last_action_at=raw.get("x1_last_action_at"),
        last_x1_trace=raw.get("last_x1_trace"),
