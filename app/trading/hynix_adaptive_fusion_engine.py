"""hynix_adaptive_fusion_engine.py — Prediction AI V2를 공통 전략 판단에 반영하되
...
행동이며, 실제 브로커 호출은 호출부(app/services/hynix_switch_engine.py)가
명시적 opt-in 토글(state["adaptive_fusion_enabled"])이 켜졌을 때 mock/real 공통
전략 판단으로 수행한다. real 모드/.env/실전 마스터 스위치는 이 모듈에서 절대 다루지 않는다.
"""
