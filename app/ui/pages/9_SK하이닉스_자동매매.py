...
st.divider()
st.header("🔄 Enhanced 자동매매 (KODEX 레버리지 ⇄ SOL 인버스2X)")
st.caption(
    "기술점수·인버스압력점수·장중모멘텀점수를 결합한 개선된 최종점수로 실제 매매 종목을 자동 스위칭합니다. "
    "당일 진입·당일 청산 원칙이며, 15:15 도달 시 두 ETF의 실제 보유수량을 조회해 전량 강제청산합니다.  \n"
    "**감시 기초자산:** SK하이닉스(000660, 시세·추세·신호 계산 전용, 직접 매매하지 않음)  \n"
    "**상승 실제 거래종목:** KODEX SK하이닉스단일종목레버리지(0193T0)  \n"
    "**하락 실제 거래종목:** SOL SK하이닉스선물단일종목인버스2X(0197X0)  \n"
    "현재가·보유수량·손익은 모두 실제 거래 ETF 기준으로 표시됩니다."
)
