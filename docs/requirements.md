...
kis:
  ...
  realized_fee_rate: 0.000036396      # 실현손익 전용 — KIS 실측(2026-09-17 실계좌)
```

> **요율이 두 개인 것은 의도적이다(2026-09-17/18).** 위 `*_fee_rate`(0.015%)는
> **전략 판정용**이다 — `worker._net_return_pct`를 거쳐 손절/익절 판정선과 모든
> 백테스트에 들어가 있어, 재백테스트 없이 바꾸면 실거래 청산 시점이 달라진다.
> 반면 **이미 체결된 거래의 기록**(거래원장 `fee` 컬럼, 일일 집계 총비용, 실현
> 손익)은 추정이 아니라 사실이므로 `realized_fee_rate` 또는 KIS가 알려준 실제
> 제비용만 쓴다(`TradeCostEngine.realized_leg_fee` 단일 경로). 실계좌 실측은
> 0.0036396%로 전략 요율의 약 1/4이다 — 2026-09-17 원장이 수수료를 15원으로 적고
> KIS는 4원이었던 사고가 이 혼용에서 나왔다. 전략 요율 변경은 별도 브랜치에서
> 재검증 후에만 한다.
