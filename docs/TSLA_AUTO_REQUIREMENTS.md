...
2026-08-01 기준 TSLA_AUTO 실거래와 replay의 비용 표시 기준은 다음과 같다.

- 매수 거래수수료: 체결금액의 0.25%
- 매도 거래수수료: 체결금액의 0.25%
- 환전우대: 95%로 계산하며, 기본 1.00% 환전 스프레드의 5%만 비용으로 반영한다(실효 0.05%).
- 슬리피지: 요청가와 실제 체결가가 모두 있으면 그 차이를 실제 슬리피지로 계산한다. 실제 차이를 산출할 수 없으면 편도 0.05%를 가정한다.
- 거래별 표시/저장 항목: Gross PnL, 매수수수료, 매도수수료, 슬리피지 비용, 환전비용, 총비용, Net PnL.
- 당일 요약 표시 항목: 총 거래수수료, 총 슬리피지, 총 환전비용, 총비용, Gross PnL, Net PnL, Net Return.
