...
6. [Primary MACD crossover 및 signed-B shadow](#6-primary-macd-crossover-및-signed-b-shadow)
...
MACD2는 SK하이닉스(`000660`)의 완성된 3분봉 MACD/Signal crossover만으로
...
`000660`의 완성 3분봉 MACD/Signal crossover에서만 나온다.
...
Primary 주문 신호는 KIS 차트형 표준 MACD crossover와 비교한다. signed-B는 주문권한이 없는
shadow 진단값으로만 유지한다.

---

## 6. Primary MACD crossover 및 signed-B shadow

Primary 주문 신호는 완성 3분봉 기준 MACD/Signal crossover onset이다.

**UP_RED**

- 직전 완성봉 `MACD <= Signal`
- 현재 완성봉 `MACD > Signal`
- 아래에서 위로 확정 교차한 onset 1회

**DOWN_BLUE**

- 직전 완성봉 `MACD >= Signal`
- 현재 완성봉 `MACD < Signal`
- 위에서 아래로 확정 교차한 onset 1회

조건:

- 현재 교차 확정봉은 반드시 당일 09:00 이후 완성된 3분봉이어야 한다.
- 직전 봉은 전일 마지막 완성봉일 수 있으나 전일 봉 자체는 주문/플래그로 등록하지 않는다.
- Worker 시작 전 이미 완료된 교차는 baseline으로만 저장하고 신규 주문으로 재생하지 않는다.
- signal_id는 현재 교차 확정봉의 KST 날짜와 시각에서 생성한다.
- 같은 방향 반복 신호는 금지하며 실제 반대 교차 이후에만 같은 방향 onset을 다시 허용한다.

signed-B는 **Signed-B shadow**로만 표시한다. 주문·오늘 primary 플래그·운영 거래 통계에는 포함하지 않는다.
...
- 포지션이 flat이며
- warm-up 및 시세가 정상이고
- Worker 시작 이후 새로 완성된 당일 3분봉이 Primary MACD crossover 조건을 충족
...
1. 15:00 강제청산
2. 안전 손절(Stop Loss)
3. 반대 Primary MACD crossover 신호 스위칭
4. Profit Lock
5. 보유 유지
...
- Profit Lock 조건에 도달하지 않았더라도, 반대 Primary MACD crossover 신호가 발생하면
...
- 새로운 반대 Primary MACD crossover 신호가 발생해야 새 episode를 시작할 수 있다.
...
- 캐시된 최신 시세 읽기
- 새 완성 3분봉 확인
- 새 봉일 때만 Primary MACD crossover 계산
...
- Primary MACD crossover: MACD, Signal, previous/current diff, relation, latest primary flag, primary signal_id
- Signed-B shadow: 최근 Histogram 3개, shadow flag, 주문권한 없음 표시
- 현재 보유 종목·수량·평단
- 현재 순손익률
- Profit Lock 상태
...
- MACD 수식
- 3분봉 경계 판정
- Primary MACD crossover 신호(`UP_RED`/`DOWN_BLUE`/`HOLD`)
- 동일 방향 반복 신호 차단
- `signal_id` 생성 규칙
- SL / Profit Lock 조건
- 15:00 청산
...
- KIS 차트의 MACD crossover 화살표와 시스템 Primary crossover 시각을 비교한다.
- 차이가 있으면 봉 경계, 정규장/NXT 포함, EMA 초기화, 미완성봉 포함 여부를 분석한다.
- 시각을 맞추기 위해 미완성봉을 사용하거나 임의 보정하지 않는다.
...
- 임시 진단/`python -c`/수동 스크립트도 동일하게 적용된다: `STATE_PATH`/
  `SIGNAL_LEDGER_PATH`/`EXECUTION_LEDGER_PATH`를 임시 디렉터리로 명시 주입하고
  fake broker/fake market data만 사용하며, 임시 경로를 지정하지 않은 진단
  스크립트는 실행하지 않는다(2026-07-24 사고: 격리 없는 진단 스크립트가 실제
  `data/logs/macd2_signal_ledger.csv`에 테스트 행을 기록해 발견 즉시 삭제한
  전례) — 운영 코드(`state_store.py`/`ledger.py`)에 이를 강제하는 가드/환경변수는
  추가하지 않는다(오탐 시 실거래 기록을 막을 위험이 더 크다고 판단).
...
| Primary 신호 | 완성 3분봉 MACD/Signal crossover: `MACD <= Signal` 후 `MACD > Signal` → `UP_RED`, `MACD >= Signal` 후 `MACD < Signal` → `DOWN_BLUE`, onset만 신규 플래그 |
| signed-B shadow | 기존 `h0,h1>0`&`d0,d1>0`, `h0,h1<0`&`d0,d1<0` 계산은 진단 표시 전용, 주문권한 없음 |
...
- [ ] Primary MACD crossover 플래그(`UP_RED`/`DOWN_BLUE`/`HOLD`) 구현
- [ ] Signed-B shadow 진단 표시 구현
...
main으로 푸쉬하지 말고, main-MACD2 브랜치로 푸쉬하세요. 
