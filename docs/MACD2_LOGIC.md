...
- Worker tick의 진행봉 start/end는 현재 KST를 09:00 기준 3분 단위로 내림해 매번 다시 저장한다. 예: 14:47 tick은 14:45~14:48 진행봉이다.
- `000660` quote와 최근 1분봉 close가 10배 스케일 차이를 보이면 MarketData 계층에서 quote를 1회 정상화하고 진단값에 남긴다. 신호 계산부에서 임의 숫자 보정은 하지 않는다.
...
- Worker input `now`, 최신 `000660` quote, 마지막 1분봉 시각/close, 가격 스케일 진단
- provisional MACD
- provisional Signal
- provisional diff
- provisional flag
