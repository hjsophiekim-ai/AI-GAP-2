...
ET = config.ET
REGULAR_MARKET_STATE = market_session.get_us_market_state(datetime(2026, 7, 24, 10, 0, tzinfo=ET))
...
def test_flat_entry_up_red_buys_tsll():
    ...
    outcome = order_executor.execute_signal(
        ...
        position=None, budget_usd=10_000.0, market_state=REGULAR_MARKET_STATE,
    )
...
def test_duplicate_signal_id_blocked_without_broker_call():
    ...
    outcome = order_executor.execute_signal(
        ...
        position=None, budget_usd=10_000.0, processed_signal_ids=frozenset({"sig-dup"}),
    )
...
def test_forbidden_symbol_never_reaches_broker(monkeypatch):
    ...
    outcome = order_executor.execute_signal(
        ...
        position=None, budget_usd=10_000.0,
    )
    assert outcome.final_state == SignalState.BLOCKED
...
def test_already_holding_same_direction_blocks_without_broker_call():
    ...
    outcome = order_executor.execute_signal(
        ...
        position=position, budget_usd=10_000.0,
    )
    assert outcome.final_state == SignalState.BLOCKED
...
def test_opposite_switch_sells_before_buying():
    ...
    outcome = order_executor.execute_signal(
        ...
        position=position, budget_usd=10_000.0, strategy_owned_qty=10, market_state=REGULAR_MARKET_STATE,
    )
    assert outcome.final_state == SignalState.EXECUTED
...
def test_strategy_ownership_mismatch_blocks_switch_before_sell():
    ...
    outcome = order_executor.execute_signal(
        ...
        position=position, budget_usd=10_000.0, strategy_owned_qty=0,
    )
...
def test_partial_fill_reflects_only_filled_qty_and_unfilled_qty_is_cancelled():
    ...
    outcome = order_executor.execute_signal(
        ...
        position=None, budget_usd=10_000.0, reconcile_retries=2, reconcile_delay_sec=0.0,
        market_state=REGULAR_MARKET_STATE,
    )
