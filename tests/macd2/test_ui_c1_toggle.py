...
def _set_mode(*, h50: bool = False, x2lite: bool = False, tw2_3slot: bool = False) -> None:
    ...
    if not (h50 or x2lite):
        state.c1_peak_protection_enabled = False
    state_store.save_state(state)


# ── 렌더 / 기본값 ─────────────────────────────────────────────────────────
def test_toggle_renders_and_defaults_off_on_n1_family():
    _set_mode(h50=True)
    at = _run(_fresh_app())
    cb = _toggle(at)
...
def test_toggle_is_disabled_outside_n1_family():
    _set_mode(tw2_3slot=True)
    at = _run(_fresh_app())
    cb = _toggle(at)
    assert cb.disabled is True, "N1 계열이 아니면 비활성이어야 한다"
    assert cb.value is False
    assert any("N1 계열)을 켜야" in c.value for c in at.caption), (
        f"자동 비활성 안내 캡션이 없다: {[c.value for c in at.caption]!r}")


def test_toggle_renders_below_the_h50_toggle():
    _set_mode(h50=True)
    at = _run(_fresh_app())
    labels = [c.label for c in at.checkbox]
    assert _LABEL in labels
    assert labels.index(_LABEL) > labels.index(config.H50_3SLOT_STRATEGY_NAME), (
        f"C1 토글이 H50 토글보다 위에 있다: {labels!r}")
...
def test_switching_away_from_n1_family_clears_the_widget_state():
    _set_mode(h50=True)
    at = _run(_fresh_app())
    _toggle(at).check().run()
    _guard(at)
...
def test_diagnostic_caption_shows_thresholds_and_arm_state():
    ...
    state.time_window_active_mode = "X2LITE_H50_3SLOT"
