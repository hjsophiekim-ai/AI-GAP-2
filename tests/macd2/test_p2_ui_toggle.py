"""P2 사이징 UI 토글 — 2026-09-21.

`MACD2_SIZING_MODE` 환경변수로만 켤 수 있던 P2 를 화면에서 켜고 끌 수 있게 한다.
토글은 **사이징 전용**이고, N1 + C1 이 둘 다 켜져 있을 때만 유효하다.

우선순위: state 토글 **또는** env 중 하나라도 P2 면 P2.
...
def _state(*, n1=True, c1=True, p2=False) -> RuntimeState:
...
def test_default_is_off():
    assert RuntimeState().p2_sizing_enabled is False
    s = _state()
...
def test_toggle_on_activates_p2_multipliers():
    s = _state(p2=True)
    assert PS.sizing_mode(s) == config.SIZING_MODE_P2
...
def test_env_alone_still_works(monkeypatch):
    ...
    _Knobs(monkeypatch).env(config.SIZING_MODE_P2)
    s = _state(p2=False)
...
def test_sizing_mode_without_state_falls_back_to_env(monkeypatch):
    ...
    _Knobs(monkeypatch).env(config.SIZING_MODE_P2)
    assert PS.sizing_mode() == config.SIZING_MODE_P2
...
def test_toggle_on_is_refused_without_n1_and_c1(n1, c1):
    _state(n1=n1, c1=c1)
    res = sv.Macd2Service().set_p2_sizing_enabled(True)
    assert res["ok"] is False
    assert res["reason"] == "P2_REQUIRES_N1_AND_C1"
    assert state_store.load_state().p2_sizing_enabled is False


def test_toggle_on_succeeds_with_n1_and_c1():
    _state(n1=True, c1=True)
    res = sv.Macd2Service().set_p2_sizing_enabled(True, changed_by="test")
...
def test_toggle_off_always_allowed():
    ...
    res = sv.Macd2Service().set_p2_sizing_enabled(False)
    assert res["ok"] is True
    assert state_store.load_state().p2_sizing_enabled is False
...
def test_toggle_survives_state_roundtrip():
    s = _state(p2=True)
    s.p2_sizing_enabled_by = "ui"
    state_store.save_state(s)
    after = state_store.load_state()
    assert after.p2_sizing_enabled is True
    assert after.p2_sizing_enabled_by == "ui"
    assert PS.sizing_mode(after) == config.SIZING_MODE_P2
...
def test_missing_field_in_old_state_file_defaults_to_off():
    ...
    raw.pop("p2_sizing_enabled", None)
...
def test_ui_renders_the_toggle_and_guards_it():
    src = _ui_src()
    assert 'key="macd2_p2_sizing_toggle"' in src
    assert "service.set_p2_sizing_enabled(" in src
    # N1 + C1 둘 다 있어야 활성
    assert "_p2_ready = _p2_n1 and _p2_c1" in src
    assert "disabled=not _p2_ready" in src
    # 조건 미충족이면 세션 상태까지 꺼 둔다(체크가 남아 보이지 않게)
    assert 'st.session_state["macd2_p2_sizing_toggle"] = False' in src
    # env 강제 ON 은 화면에 알린다
    assert "MACD2_SIZING_MODE=P2" in src


def test_ui_help_states_the_grade_honestly():
    """등급을 PROMISING 으로 적고 ADOPT 라고 하지 않는다."""
    src = _ui_src()
    i = src.index('key="macd2_p2_sizing_toggle"')
    seg = src[max(0, i - 3000):i + 3000]
    assert "PROMISING" in seg
    assert "placebo 94.56" in seg
    assert "74.7%" in seg          # uplift 중 레버리지 비중
