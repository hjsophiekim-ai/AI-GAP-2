...
G. Exit-mode tier (2026-08-10 사용자 요청) — sideways_filter_enabled no
   longer touches profit_lock_enabled/quick_profit_enabled at all (the prior
   forced-OFF and, before that, 09:00-11:00/11:00+ time-of-day auto-switch
   were both removed); both stay under ordinary manual UI control regardless
   of whether the mode is ON or OFF.
...
# ══════════════════════════════════════════════════════════════════════════
# G. Exit-mode tier while 추세전환장 mode is ON (2026-08-10 사용자 요청: 추세전환장
#    ON일 때도 Profit Lock/Quick Profit을 사용자가 직접 켜고 끌 수 있어야 한다 —
#    이전의 강제 OFF/시간대별 자동전환 로직은 모두 제거, run_once는 이제 두
#    토글을 전혀 건드리지 않는다)
# ══════════════════════════════════════════════════════════════════════════
def test_run_once_leaves_exit_filters_under_manual_control_when_sideways_enabled():
    """추세전환장 mode ON이어도 run_once는 profit_lock_enabled/quick_profit_
    enabled을 사용자가 마지막으로 설정한 값 그대로 둔다 (강제 OFF 없음)."""
    now = _at(10, 0)
    df_1m = _1m_from_3m_closes(now - timedelta(minutes=300), [100.0] * 100)
