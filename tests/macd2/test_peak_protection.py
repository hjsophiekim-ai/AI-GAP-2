...
def test_is_active_requires_toggle_and_n1_mode():
    """2026-09-20: N1 이 production 모드로 들어오면서 허용 모드를 **N1 만**
    으로 좁혔다. X2-lite/H50 은 TP2 가 5.0% 라 C1 이 구조적으로 발동 불가."""
    s = _state(time_window_n1_filter_enabled=True)
    assert pp.is_active(s) is False                 # 토글 OFF
    s.c1_peak_protection_enabled = True
    assert pp.is_active(s) is True                  # N1 에서만 활성
    for other in ("time_window_h50_filter_enabled",
                  "time_window_x2lite_filter_enabled",
                  "time_window_3slot_filter_enabled",
                  "time_window_twf_filter_enabled"):
        s2 = _state(c1_peak_protection_enabled=True, **{other: True})
        assert pp.is_active(s2) is False, other
        assert pp.is_enabled(s2) is True, other     # 토글 자체는 켜져 있음


def test_kill_switch_disables_everything(monkeypatch):
    monkeypatch.setattr(config, "C1_ENABLED", False)
    s = _state(time_window_n1_filter_enabled=True, c1_peak_protection_enabled=True)
...
def test_note_peak_is_monotonic_and_clear_resets_only_position_state():
    s = _state(c1_peak_protection_enabled=True, time_window_n1_filter_enabled=True)
...
def test_stored_on_is_forced_off_outside_n1():
    s = _state(time_window_n1_filter_enabled=True, c1_peak_protection_enabled=True)
    raw = state_store.serialize(s)
    raw["time_window_n1_filter_enabled"] = False
...
def test_supported_mode_is_n1_only():
    for flag, expected in (("time_window_n1_filter_enabled", True),
        ...
        if expected:
            assert tw3.active_3slot_mode(s) in tw3.MODES_N1_FAMILY
