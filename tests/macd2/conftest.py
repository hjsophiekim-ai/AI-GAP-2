...
from app.trading.macd2 import config, ledger, market_data, service, state_store, worker_lock
...
def _isolate_macd2_state(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(worker_lock, "STATE_DIR_PATH", tmp_path)
    monkeypatch.setattr(worker_lock, "LOCK_PATH", tmp_path / "macd2_worker_lock.json")
